...
type ProductCardProps = Omit<IProduct, "description">;
...
const ProductCard: FC<ProductCardProps> = ({ id, name, modelPath, price, groundColor, config3D }): JSX.Element => {
    return (
        <article className="product-card relative cursor-pointer rounded-lg h-[260px] overflow-hidden" style={{ backgroundColor: `${groundColor}` }}>
            <ProductViewCanvas modelPath={modelPath!} config3d={config3D} />
            ...
            <div className="product-card__active absolute top-0 z-40 w-full h-full flex items-center justify-center">
                ...
                <div className="content flex flex-col space-y-3 relative z-20 items-center text-center">
                    <h5 className="text-xl text-white font-michroma">{name}</h5>
                    <p className="text-lg text-white">{formatIntoPrice(price!)} Euros</p>
