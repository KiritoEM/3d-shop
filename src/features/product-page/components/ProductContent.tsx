...
const ProductContent: FC<ProductContentProps> = ({ cuid }): JSX.Element => {
    ...
    if (data?.customisationConfigs && Object.keys(data.customisationConfigs).length > 0) {
        selectedMaterial = data?.customisationConfigs.colorCustomisation.find((opt) => opt.color === color.variant)?.materials ?? {}
    }
                ...
                !error ? (
                    <Fragment>
                        <div className="product-content__3d h-[68vmin] sm:h-[450px] lg:h-[70vmin] xl:h-[77vmin] 2xl:h-[72vmin] w-full lg:w-[72vmin] xl:w-[80vmin] 2xl:w-[75vmin] rounded-xl overflow-hidden" style={{ backgroundColor: `${data?.groundColor}` }}>
                            {
                                !isLoading ?
                                    (
                                        <ProductViewCanvas config3D={data?.config3D!} modelPath={data?.modelPath!} orbitControl selectedMaterials={selectedMaterial ?? {}} />
                                    ) : (
                        ...
                        <ProductDetails
                            ...
                            changeModelColor={(variant: string) => setColor({ variant: variant })}
